...
// TODO: move to a new file in the prelude folder
const unionWith = (f, m1, m2) => {
    const r = new Map();
    Array.from(m1.entries()).forEach(([k, v]) => m2.has(k) ? r.set(k, f(v, m2.get(k))) : r.set(k, v));
    Array.from(m2.entries()).forEach(([k, v]) => !m1.has(k) && r.set(k, v));
    return r;
};
...
const summary = st => {
    ...
    // console.log(`st.collected.length: ${st.collected.length}`);
    // st.collected.forEach(col => console.log(`col: ${JSON.stringify(Array.from(col.entries()))}`));
    const s2s = st.collected
        .map(s => Array.from(s).filter(t => st.labels.has(t) && st.labels.get(t) === 0))
        .filter(s2 => s2.length)
        .map(s2 => s2.join(', '));
    // console.log(`s2s: ${JSON.stringify(s2s)}`);
    const v1 = s2s.sort();
    const v2 = group(v1);
