...
export const shrinking = (shrinker, x0, pf) => {
    ...
    const v1 = props(x0); // :: Rose (Gen Prop) where Prop is effectively Rose Result
    const v2 = promote(v1); // :: Gen (Rose Prop) where Prop is effectively Rose Result
    const v3 = v2.map(r => r.map(p => MkProp(joinRose(p.unProp))));
...
// promote :: Monad m => m (Gen a) -> Gen (m a)
    ...
    // r :: Prop
    // m r :: Rose Prop
const promote = m => delay().flatMap(_eval => constant(liftMgen(_eval, m)));

// delay :: Gen (Gen a -> a)
const delay = () => MkGen((r, n) => g => g.run(r, n));

// liftM :: (Monad m) => (a1 -> r) -> m a1 -> m r
const liftMgen = (f, m1) => m1.flatMap(x1 => constant(f(x1)));

// noShrinking :: Testable prop => prop -> Property
export const noShrinking = p => mapRoseResult(onRose((res /* _ */) => MkRose(res, [])), p);
...
export const counterexample = (s, p) =>
    mapTotalResult(res =>
        res.set('testCase', [s, ...res.testCase]),
        callback(
            new PostFinalFailure(CallbackKind.Counterexample, (st /* _res */) => putLine(st.terminal, s)),
            p));
