...
import { forAll, withMaxSuccess, verbose, label, classify, noShrinking } from './property';
...
quickCheckWith(stdArgs.set('maxSuccess', 15), forAll(genInt, prop_ofInt_pass));
...
const result3 = quickCheckResult(forAll(listOf(genInt), prop_ofIntList_fail));
console.log(`result3: ${JSON.stringify(result3)}`);

const result4 = quickCheckResult(forAll(listOf(genInt), compose(noShrinking, prop_ofIntList_fail)));
