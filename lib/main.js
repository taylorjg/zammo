...
import { quickCheck, quickCheckResult, quickCheckWithResult, verboseCheckResult, stdArgs } from './test';
...
const result15 = quickCheckResult(withMaxSuccess(10, forAll(suchThat(genInt, n => n >= 0 && n % 2 === 0), prop_Good4)));
console.log(`result15: ${JSON.stringify(result15)}`);

const result16 = quickCheckResult(withMaxSuccess(10, forAll(suchThatMap(genInt, n => n >= 0 ? Just(n) : Nothing), prop_Good4)));
console.log(`result16: ${JSON.stringify(result16)}`);

const result17 = quickCheckResult(forAll(listOf(genInt), ns => classify(ns.length > 1, 'non-trivial', ns.every(n => n + n === 2 * n))));
console.log(`result17: ${JSON.stringify(result17)}`);
